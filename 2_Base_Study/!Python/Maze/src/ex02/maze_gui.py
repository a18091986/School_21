...
class MazeApp:
    ...
    def __init__(self, root):
        ...
        self.eller_button = tk.Button(root, text="Eller Maze", command=self.generate_eller_maze)
        self.eller_button.pack(side=tk.RIGHT)
    ...
    def generate_eller_maze(self):
        """Generates a perfect maze using Eller's algorithm."""
        n_rows = simpledialog.askinteger("Input", "Number of rows:", minvalue=1, maxvalue=50)
        n_cols = simpledialog.askinteger("Input", "Number of columns:", minvalue=1, maxvalue=50)
        if n_rows and n_cols:
            generator = EllerMazeGenerator(n_rows, n_cols)
            vertical_walls, horizontal_walls = generator.generate_maze()
            self.maze = Maze(n_rows, n_cols, vertical_walls, horizontal_walls)
            self.draw_maze()
