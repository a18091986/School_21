...
int s21_eq_matrix(matrix_t *A, matrix_t *B) {
  int res = SUCCESS;
  if (is_matrix_correct_check(A) != OK || is_matrix_correct_check(B) != OK ||
      A->columns != B->columns || A->rows != B->rows)
    res = FAILURE;
  else {
    for (int i = 0; i < A->rows; i++) {
      for (int j = 0; j < A->columns; j++) {
        if (fabs(A->matrix[i][j] - B->matrix[i][j]) > PRECISION) res = FAILURE;
      }
    }
  }
  ...
  if (A->rows == 2) {
    ...
    for (int i = 0; i < A->rows; i++) {
      ...
      res += pow((-1), i) * A->matrix[0][i] *
             recursive_determinant_calc(&minor_matrix);
...
int s21_calc_complements(matrix_t *A, matrix_t *result) {
  ...
  if (!is_matrix_correct_check(A)) {
    ...
    if (A->rows == A->columns) {
      ...
      if (!res) res = complements_calculation(A, result);
    }
  }
  return res;
}
    ...
    for (int i = 0; i < A->rows; i++) {
      for (int j = 0; j < A->columns; j++) {
        ...
        if (!res) {
          ...
          if (!res) result->matrix[i][j] = pow((-1), i + j) * determinant;
        }
        s21_remove_matrix(&minor_matrix);
      }
    }
